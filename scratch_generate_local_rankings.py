...
for rel, items in religions_data.items():
    ...
    limit = 150 if rel != "Buddhism" else 150
    top_items = [x[1] for x in scored_items[:limit]]
    ai_candidates.extend(top_items)

total_ai_verses = len(ai_candidates)
print(f"Total candidate verses selected for local AI ranking: {total_ai_verses}", flush=True)

# Batch size of 20 for local LLM (reduced from 30 to prevent context limit issues)
BATCH_SIZE = 20
...
llm = Llama(model_path=model_path, n_ctx=4096, n_threads=6, n_gpu_layers=-1)

print(f"Running local AI ranking over {len(batches)} batches...", flush=True)
ai_rankings = {}
...
You are an expert in spiritual and religious texts. Rate the peacefulness, wisdom, and inspiring quality of each verse on a scale of 1 to 10 (10 is extremely peaceful/wise/inspiring, 1 is dry administrative details, genealogy lists, or violent context).
Respond ONLY with a flat JSON object mapping the ID to the integer rank. Do not include any explanations or markdown blocks.
Example: {{"v_1": 9}}<|im_end|>
<|im_start|>user
Rate these:
{json.dumps(batch, indent=2)}<|im_end|>
<|im_start|>assistant
"""
    ...
    for attempt in range(3):
        try:
            # Generate JSON output. Qwen is smart enough to output exactly JSON if prompted.
            res = llm(prompt, max_tokens=1000, temperature=0.0)
            text_resp = res["choices"][0]["text"].strip()
            
            # Clean response if LLM accidentally wrapped it in markdown code block
    ...
    if not success:
        print(f"    WARNING: Batch {idx+1} failed completely after 3 attempts.", flush=True)

# Merge rankings: prioritize Local AI rankings, fallback to baseline rankings
final_rankings = {}
...
print(f"Local AI-refined ranks size: {len(ai_rankings)}", flush=True)

with open(output_path, "w", encoding="utf-8") as f:
    json.dump(final_rankings, f, indent=2)

print(f"SUCCESS: Saved all local AI-evaluated rankings to {output_path}", flush=True)
